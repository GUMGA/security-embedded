let TEMPLATE = `
          ...
          <div class="col-xs-12">
            <div class="row">
                <div class="col-md-12">
                    ...
                    <div ng-if="organizationMessage" align="center">
                      <label ng-if="organizationMessage.status == 'error'" class="text text-danger">{{organizationMessage.message}}</label>
                      <br/>
                      <button ng-if="organizationMessage.status == 'error'" ng-click="getOrganization()" class="gmd btn btn-primary">Tentar novamente</button>
                      <label ng-if="organizationMessage.status == 'loading'" class="text text-muted">{{organizationMessage.message}}</label>
                    </div>

                    <div ui-tree="vm.treeOptions" data-max-depth="1" id="tree-root" class="angular-ui-tree">
                        <ol ui-tree-nodes ng-model="organtization"
                            class="angular-ui-tree-nodes">
                            <li ui-tree-node class="angular-ui-tree-node" data-nodrag ng-repeat="org in organtization"
  ...
  h64v64H32V192z M32,320h64v64H32V320z M288,160h224v32H288V160z M288,512h64V384h96v128h64V224H288V512z" style="fill: #54545a;"/>
    ...
    <div class="col-xs-12" ng-if="view == 'list-users'">
        <div class="gmd panel panel-default">
          <div class="panel-body">
            ...
            <div ng-if="usersMessage" align="center">
              <label ng-if="usersMessage.status == 'error'" class="text text-danger">{{usersMessage.message}}</label>
              <br/>
              <button ng-if="usersMessage.status == 'error'" ng-click="getUsersByOrganization(variables.organizationSelected.id)"
              class="gmd btn btn-primary">Tentar novamente</button>
              <label ng-if="usersMessage.status == 'loading'" class="text text-muted">{{usersMessage.message}}</label>
            </div>
    ...
    <div class="col-xs-12" ng-if="view == 'edit-user' || view == 'new-user'">
        <div class="gmd panel panel-default">
          ...
          <div ng-if="userMessage" align="center">
            <label ng-if="userMessage.status == 'error'" class="text text-danger">{{userMessage.message}}</label>
            <br/>
            <button ng-if="userMessage.status == 'error'" ng-click="getUserByEmail(variables.userSelected.login)"
            class="gmd btn btn-primary">Tentar novamente</button>
            <label ng-if="userMessage.status == 'loading'" class="text text-muted">{{userMessage.message}}</label>
          </div>
